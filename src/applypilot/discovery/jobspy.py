...
def _run_one_search(
    ...
    # Filter by description keywords (reject contractor/marketplace postings)
    description_reject = defaults.get("description_reject", [])
    filtered_desc = 0
    if description_reject and "description" in df.columns:
        def _desc_ok(desc: str) -> bool:
            if not desc or desc == "nan":
                return True
            d = desc.lower()
            return not any(kw.lower() in d for kw in description_reject)
        before_desc = len(df)
        df = df[df["description"].apply(lambda d: _desc_ok(str(d)))]
        filtered_desc = before_desc - len(df)

    # Filter by title seniority (reject senior/staff/lead/etc.)
    title_reject = defaults.get("title_reject", [])
    filtered_title = 0
    if title_reject and "title" in df.columns:
        def _title_ok(title: str) -> bool:
            if not title or title == "nan":
                return True
            t = title.lower()
            return not any(kw.lower() in t for kw in title_reject)
        before_title = len(df)
        df = df[df["title"].apply(lambda t: _title_ok(str(t)))]
    ...
    if filtered_desc:
        msg += f", {filtered_desc} filtered (contractor)"
    if filtered_title:
        msg += f", {filtered_title} filtered (seniority)"
    log.info(msg)

    return {"new": new, "existing": existing, "errors": 0,
            "filtered": filtered_loc + filtered_desc + filtered_title,
            "total": before, "label": label}
