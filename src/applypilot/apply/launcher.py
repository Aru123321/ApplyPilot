...
def acquire_job(target_url: str | None = None, min_score: int = 7,
    ...
    try:
        ...
        if target_url:
            ...
            url_clauses = ""
            if blocked_patterns:
                # Filter both the discovery URL and the direct application URL
                url_clauses = " ".join(
        ...
        from applypilot.config import is_manual_ats, load_sites_config
        ...
        # Skip contractor/marketplace postings detected via description keywords
        desc_reject = load_sites_config().get("description_reject", [])
        desc_text = ((row["full_description"] or "") if "full_description" in row.keys() else "").lower()
        if desc_reject and desc_text and any(kw.lower() in desc_text for kw in desc_reject):
            conn.execute(
            ...
            conn.commit()
            logger.info("Skipping contractor/marketplace posting: %s", row["url"][:80])
            return None
